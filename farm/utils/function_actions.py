...
logger = logging.getLogger("farm.function_actions")
...
def action_system_restart(message):
    ...
    cmd = '{path}/farm/scripts/farm_wrapper restart 2>&1'.format(
...
def action_system_shutdown(message):
    ...
    cmd = '{path}/farm/scripts/farm_wrapper shutdown 2>&1'.format(
        path=INSTALL_DIRECTORY)
    subprocess.Popen(cmd, shell=True)
    return message
...
def trigger_action(
    ...
    logger_actions = logging.getLogger("farm.trigger_action_{id}".format(
...
def trigger_function_actions(function_id, message='', debug=False):
    ...
    :return:
    """
    logger_actions = logging.getLogger("farm.trigger_function_actions_{id}".format(

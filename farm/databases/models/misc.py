...
class Misc(CRUDMixin, db.Model):
    ...
    enable_upgrade_check = db.Column(db.Boolean, default=True)  # Periodically check for a Farm upgrade
    farm_upgrade_available = db.Column(db.Boolean, default=False)  # Stores if an upgrade is available

...
from farm.farm_flask.utils.utils_general import choices_custom_functions
from farm.farm_flask.utils.utils_general import generate_form_input_list
from farm.farm_flask.utils.utils_general import generate_form_output_list
from farm.tests.software_tests.conftest import login_user
...
@mock.patch('farm.farm_flask.routes_authentication.login_log')
...
def test_api_docs_when_not_logged_in(testapp):
    ...
    assert 'Farm API' in response, "Unexpected HTTP Response: \n{body}".format(body=response.body)
...
def create_user(farm_db, role_id, name, password):
    """ Create fake admin user """
    new_user = UserFactory()
    new_user.name = name
    new_user.set_password(password)
    new_user.role_id = role_id
    farm_db.add(new_user)
    farm_db.commit()
    return new_user

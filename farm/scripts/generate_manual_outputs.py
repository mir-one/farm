...
from farm.scripts.generate_doc_output import generate_controller_doc
...
if __name__ == "__main__":
    for output_id, output_data in parse_output_information(exclude_custom=True).items():
        ...
        if ('output_manufacturer' in output_data and
                output_data['output_manufacturer'] in ['Linux', 'Farm', 'Raspberry Pi', 'System']):

            if name_str in farm_info and 'dependencies_module' in farm_info[name_str]:
                # Multiple sets of dependencies, append library
                farm_info[name_str]['dependencies_module'].append(output_data['dependencies_module'])
            else:
                # Only one set of dependencies
                farm_info[name_str] = output_data
                if 'dependencies_module' in output_data:
                    farm_info[name_str]['dependencies_module'] = [output_data['dependencies_module']]  # turn into list
        else:
    ...
    farm_info = dict(OrderedDict(sorted(farm_info.items(), key = lambda t: t[0])))
    outputs_info = dict(OrderedDict(sorted(outputs_info.items(), key = lambda t: t[0])))

    list_outputs = [
        (farm_info, "Built-In Outputs (System)"),

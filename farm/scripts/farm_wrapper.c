...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) {
    if (strcmp(argv[1], "backup-create") == 0) {
      ...
      sprintf(cmd, "rm -rf /var/Farm-backups/Farm-backup-%s", argv[2]);
      system(cmd);
      ...
      sprintf(cmd, "service farm restart");
      system(cmd);
    } else if (strcmp(argv[1], "daemon_start") == 0) {
      sprintf(cmd, "service farm start");
      system(cmd);
    } else if (strcmp(argv[1], "daemon_stop") == 0) {
      sprintf(cmd, "service farm stop");
      system(cmd);
    } else if (strcmp(argv[1], "delete_upload_dir") == 0) {
      sprintf(cmd, "rm -rf /var/farm-root/upload");
      system(cmd);
    } else if (strcmp(argv[1], "frontend_reload") == 0) {
      sprintf(cmd, "service farmflask reload");
      system(cmd);
    } else if (strcmp(argv[1], "frontend_restart") == 0) {
      sprintf(cmd, "service farmflask restart");
      system(cmd);
      ...
      system(cmd);
    } else if (strcmp(argv[1], "influxdb_restore_farm_db") == 0 && (argc > 2)) {
      sprintf(cmd, "influxd restore -portable -db farm_db -newdb farm_db_bak %s", argv[2]);
      system(cmd);
    ...
    printf("Farm Wrapper: A wrapper to allow the farm web interface\n");
    printf("                to perform superuser actions.\n\n");
    printf("Usage: farm_wrapper [command]\n\n");
    printf("Commands:\n");
    printf("   backup-create:              Create Farm backup\n");
    printf("   backup-delete [DIRECTORY]:  Delete Farm backup [DIRECTORY]\n");
    printf("   backup-restore [DIRECTORY]: Restore Farm from backup [DIRECTORY]\n");
    printf("   daemon_restart:             Restart the farm daemon in normal mode\n");
    printf("   daemon_restart_debug:       Restart the farm daemon in debug mode\n");
    printf("   daemon_start:               Start the farm daemon\n");
    printf("   daemon_stop:                Stop the farm daemon\n");
    ...
    printf("   frontend_reload:            Reload the farm frontend\n");
    printf("   frontend_restart:           Restart the farm frontend\n");
    printf("   influxdb_start:             Start influxdb\n");
    printf("   influxdb_stop:              Stop influxdb\n");
    printf("   influxdb_restart:           Restart influxdb\n");
    printf("   influxdb_restore_farm_db  Restore influxdb database farm_db to farm_db_bak\n");
    printf("   initialize:                 Run the Farm initialization sequence\n");
    printf("   update_dependencies:        Install unmet dependencies and update installed dependencies\n");
    printf("   update_permissions:         Set the Farm file/folder permissions\n");
    printf("   restart:                    Restart the computer after a 10 second pause\n");
    printf("   shutdown:                   Shutdown the computer after a 10 second pause\n");
    printf("   upgrade-release-wipe:       Upgrade Farm to the latest major version on github and wipe database and virtualenv\n");
    printf("   upgrade-release-major:      Upgrade Farm to the latest major version on github\n");
    printf("   upgrade-master:             Upgrade Farm to the latest master branch on github\n");
  }

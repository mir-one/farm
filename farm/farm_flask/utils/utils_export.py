...
from farm.config import DATABASE_NAME
...
from farm.utils.tools import create_measurements_export
from farm.utils.tools import create_settings_export
...
def export_settings(form):
    """
    Save the Farm settings database (farm.db) to a zip file and serve it
    to the user
    """
    action = '{action} {controller}'.format(
        action=TRANSLATIONS['export']['title'],
    ...
    try:
        ...
        if not status:
            return send_file(
                ...
                as_attachment=True,
                attachment_filename=
                    'Farm_{mver}_Settings_{aver}_{host}_{dt}.zip'.format(
                        mver=FARM_VERSION, aver=ALEMBIC_VERSION,
...
def export_influxdb(form):
    """
    Save the Farm InfluxDB database in the Enterprise-compatible format, zip
        ...
        if not status:
            ...
            if influxd_version_out:
                ...
                return send_file(
                    ...
                    as_attachment=True,
                    attachment_filename=
                    'Farm_{mv}_Influxdb_{iv}_{host}_{dt}.zip'.format(
                        mv=FARM_VERSION, iv=influxd_version,
                        host=socket.gethostname().replace(' ', ''),
                        dt=datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")))
            else:
...
def import_settings(form):
    """
    Receive a zip file containing a Farm settings database that was
    exported with export_settings(), then back up the current Farm settings
    ...
    try:
        ...
        tmp_folder = os.path.join(upload_folder, 'farm_db_tmp')
        full_path = None
        ...
        if not form.settings_import_file.data:
            ...
            try:
                ...
                elif name_split[1] > FARM_VERSION:
                    error.append(
                        "Invalid Farm version: {fv} > {mv}. "
        ...
        if not error:
            try:
                # Stop Farm daemon (backend)
                cmd = "{pth}/farm/scripts/farm_wrapper " \
                      "daemon_stop".format(
                    pth=INSTALL_DIRECTORY)
                _, _, _ = cmd_output(cmd)

                # Backup current database and replace with extracted farm.db
                imported_database = os.path.join(tmp_folder, DATABASE_NAME)
                backup_name = (
                    SQL_DATABASE_FARM + '.backup_' +
                    datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
                os.rename(SQL_DATABASE_FARM, backup_name)  # rename current database to backup name
                os.rename(imported_database, SQL_DATABASE_FARM)  # move zipped database to Farm
...
def thread_import_influxdb(tmp_folder):
    ...
    try:
        client.drop_database(farm_db_backup)
    except Exception as msg:
        print("Error while deleting db prior to restore: {}".format(msg))

    # Restore the backup to new database farm_db_bak
    try:
        logger.info("Creating tmp db with restore data")
        command = "{pth}/farm/scripts/farm_wrapper " \
                  "influxdb_restore_farm_db {dir}".format(
    ...
    try:
        ...
        command = "{pth}/farm/scripts/farm_wrapper delete_upload_dir".format(
            pth=INSTALL_DIRECTORY)
        cmd = subprocess.Popen(
...
def import_influxdb(form):
    ...
    try:
        correct_format = 'Farm_FARMVERSION_Influxdb_INFLUXVERSION_HOST_DATETIME.zip'
        upload_folder = os.path.join(INSTALL_DIRECTORY, 'upload')
        tmp_folder = os.path.join(upload_folder, 'farm_influx_tmp')
        full_path = None

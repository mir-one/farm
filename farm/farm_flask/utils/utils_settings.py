...
from farm.config_devices_units import UNITS
from farm.config_translations import TRANSLATIONS
from farm.databases import set_api_key
...
from farm.farm_flask.utils.utils_general import choices_measurements
from farm.farm_flask.utils.utils_general import choices_units
...
from farm.farm_flask.utils.utils_input import input_deactivate_associated_controllers
...
from farm.utils.send_data import send_email
from farm.utils.system_pi import all_conversions
from farm.utils.system_pi import assure_path_exists
from farm.utils.system_pi import base64_encode_bytes
        ...
        if not messages["error"]:
            try:
                ...
                if force_https != form.force_https.data:
                    ...
                    wsgi_file = '{inst_dir}/farm_flask.wsgi'.format(
...
def settings_function_import(form):
    """
    Receive an controller module file, check it for errors, add it to Farm controller list
    """
    action = '{action} {controller}'.format(
    ...
    try:
        # correct_format = 'Farm_FARMVERSION_Settings_DBVERSION_HOST_DATETIME.zip'
        install_dir = os.path.abspath(INSTALL_DIRECTORY)
        tmp_directory = os.path.join(install_dir, 'farm/functions/tmp_functions')
        ...
        if not error:
            ...
            cmd = '{path}/farm/scripts/farm_wrapper frontend_reload 2>&1'.format(
                path=install_dir)
...
def settings_input_import(form):
    """
    Receive an input module file, check it for errors, add it to Farm input list
    """
    action = '{action} {controller}'.format(
        action=gettext("Import"),
    ...
    try:
        # correct_format = 'Farm_FARMVERSION_Settings_DBVERSION_HOST_DATETIME.zip'
        install_dir = os.path.abspath(INSTALL_DIRECTORY)
        tmp_directory = os.path.join(install_dir, 'farm/inputs/tmp_inputs')
        assure_path_exists(tmp_directory)
...
def settings_output_import(form):
    """
    Receive an output module file, check it for errors, add it to Farm output list
    """
    action = '{action} {controller}'.format(
        action=gettext("Import"),
    ...
    try:
        # correct_format = 'Farm_FARMVERSION_Settings_DBVERSION_HOST_DATETIME.zip'
        install_dir = os.path.abspath(INSTALL_DIRECTORY)
        tmp_directory = os.path.join(install_dir, 'farm/outputs/tmp_outputs')
        assure_path_exists(tmp_directory)
...
def settings_widget_import(form):
    """
    Receive an widget module file, check it for errors, add it to Farm widget list
    """
    action = '{action} {controller}'.format(
        action=gettext("Import"),
    ...
    try:
        # correct_format = 'Farm_FARMVERSION_Settings_DBVERSION_HOST_DATETIME.zip'
        install_dir = os.path.abspath(INSTALL_DIRECTORY)
        tmp_directory = os.path.join(install_dir, 'farm/widgets/tmp_widgets')
        assure_path_exists(tmp_directory)
...
def settings_pi_mod(form):
    ...
    if form.save_sample_rates.data:
        ...
        if form.pigpiod_sample_rate.data not in ['low', 'high',
            ...
            # Stop the Farm daemon
            cmd = "{pth}/farm/scripts/farm_wrapper daemon_stop" \
            ...
            if (form.pigpiod_sample_rate.data != 'uninstalled' and
                ...
                cmd = "{pth}/farm/scripts/farm_wrapper install_pigpiod" \
                      " | ts '[%Y-%m-%d %H:%M:%S]' 2>&1".format(
                    pth=INSTALL_DIRECTORY)
            ...
            # Disable pigpiod
            cmd = "{pth}/farm/scripts/farm_wrapper disable_pigpiod" \
                  " | ts '[%Y-%m-%d %H:%M:%S]' 2>&1".format(
                pth=INSTALL_DIRECTORY)
            ...
            if form.pigpiod_sample_rate.data == 'low':
                # Install pigpiod (sample rate of 1 ms)
                cmd = "{pth}/farm/scripts/farm_wrapper enable_pigpiod_low" \
                      " | ts '[%Y-%m-%d %H:%M:%S]' 2>&1".format(
                    pth=INSTALL_DIRECTORY)
                ...
                cmd = "{pth}/farm/scripts/farm_wrapper enable_pigpiod_high" \
                      " | ts '[%Y-%m-%d %H:%M:%S]' 2>&1".format(
                    pth=INSTALL_DIRECTORY)
                ...
                cmd = "{pth}/farm/scripts/farm_wrapper enable_pigpiod_disabled" \
                      " | ts '[%Y-%m-%d %H:%M:%S]' 2>&1".format(
                    pth=INSTALL_DIRECTORY)
                disable_pigpiod = subprocess.Popen(cmd, shell=True)
                disable_pigpiod.wait()
                ...
                cmd = "{pth}/farm/scripts/farm_wrapper uninstall_pigpiod" \
                      " | ts '[%Y-%m-%d %H:%M:%S]' 2>&1".format(
                    pth=INSTALL_DIRECTORY)
                uninstall_pigpiod = subprocess.Popen(cmd, shell=True)
                uninstall_pigpiod.wait()

            # Start the Farm daemon
            cmd = "{pth}/farm/scripts/farm_wrapper daemon_start" \
                  " | ts '[%Y-%m-%d %H:%M:%S]' 2>&1".format(
                pth=INSTALL_DIRECTORY)
        ...
        if form_mod_alert.validate():
            ...
            if form_mod_alert.send_test.data:
                send_email(
                    ...
                    "This is a test email from Farm")
...
def settings_diagnostic_delete_settings_database():
    ...
    if not error:
        try:
            os.remove('/var/farm-root/databases/farm.db')

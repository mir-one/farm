...
from farm.config import LOG_PATH
from farm.config import PATH_CAMERAS
from farm.config import PATH_NOTE_ATTACHMENTS
...
from farm.farm_flask.routes_authentication import clear_cookie_auth
from farm.farm_flask.utils import utils_general
from farm.farm_flask.utils.utils_general import get_ip_address
from farm.farm_flask.utils.utils_output import get_all_output_states
from farm.utils.database import db_retrieve_table
from farm.utils.image import generate_thermal_image_from_pixels
from farm.utils.influx import influx_time_str_to_milliseconds
from farm.utils.influx import query_string
from farm.utils.system_pi import assure_path_exists
from farm.utils.system_pi import is_int
...
def video_feed(unique_id):
    ...
    camera_stream = import_module('farm.farm_flask.camera.camera_' + camera_options.library).Camera
    ...
    try:
        ...
        if DOCKER_CONTAINER:
            if action == 'daemon_restart':
                ...
                subprocess.Popen('docker restart farm_flask 2>&1', shell=True)
                flash(gettext("Command to reload the frontend sent"), "success")
        else:
            cmd = '{path}/farm/scripts/farm_wrapper {action} 2>&1'.format(

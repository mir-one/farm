...
from farm.config import ALEMBIC_UPGRADE_POST
    ...
    for each_revision in read_revision_file():
        ...
        #         print(msg)
            ...
            try:
                ...
                with session_scope(FARM_DB_PATH) as session:
            ...
            try:
                ...
                with session_scope(FARM_DB_PATH) as cond_sess:
            ...
            try:
                ...
                with session_scope(FARM_DB_PATH) as output_sess:
            ...
            try:
                ...
                from farm.databases.models import DeviceMeasurements
                from farm.databases.models import Output
                from farm.config import OUTPUT_INFO
            ...
            try:
                ...
                from farm.databases.models import Conditional
                from farm.databases.models import Input
                from farm.inputs.python_code import execute_at_creation
                from farm.utils.conditional import save_conditional_code

                conditions = conditional_sess.query(ConditionalConditions).all()
                actions = conditional_sess.query(Actions).all()

                with session_scope(FARM_DB_PATH) as conditional_sess:
                ...
                with session_scope(FARM_DB_PATH) as input_sess:
                ...
                with session_scope(FARM_DB_PATH) as conditional_sess:
                    for each_conditional in conditional_sess.query(Conditional).all():
                        if each_conditional.conditional_statement:
                            ...
                            with session_scope(FARM_DB_PATH) as condition_sess:
